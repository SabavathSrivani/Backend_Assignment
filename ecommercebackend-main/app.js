...
app.use('/api/shipping', shippingRoutes);// logic to route ship rocket api
...
// Adding product image URLs
app.post('/add-product-image-urls', (req, res) => {
  const { productId, imageUrls } = req.body;

  // Validate that imageUrls is an array
  if (!Array.isArray(imageUrls)) {
    return res.status(400).json({ error: 'imageUrls must be an array of URLs' });
  }

  // Update the product in the database with the image URLs
  Product.findByIdAndUpdate(productId, { $push: { images: { $each: imageUrls } } }, { new: true })
    .then(updatedProduct => res.json({ message: 'Image URLs added successfully', product: updatedProduct }))
...
app.delete('/remove-product/:id', (req, res) => {
  ...
  Product.findByIdAndDelete(id)
    .then(() => res.json({ message: 'Product removed successfully' }))
    .catch(err => res.status(400).json({ error: err.message }));
});
...
app.put('/update-product-description', (req, res) => {
  const { productId, description } = req.body;

  Product.findByIdAndUpdate(productId, { description })
    .then(() => res.json({ message: 'Product description updated successfully' }))
    .catch(err => res.status(400).json({ error: err.message }));
});
...
app.post('/checkout', (req, res) => {
  ...
  if (totalAmount > 499) {
    ...
    res.json({ freeDelivery: true, discount });
  } else {
    res.json({ freeDelivery: false, discount: 0 });
  }
});
